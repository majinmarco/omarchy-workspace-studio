// Workspace Studio — vendored copy of Omarchy's application ranking.
//
// PROVENANCE: verbatim copy of
//   /usr/share/omarchy/shell/services/AppSearch.js
// from Omarchy 4.0.3 (MIT, https://github.com/basecamp/omarchy), taken on
// 2026-09-09. Only this header was added; the code below is unmodified,
// including its `module.exports` guard, which is what lets `node --test`
// cover it.
//
// WHY A COPY: the host only hands `shell.appLibrary` to plugins that declare
// the "menu" kind (shell.qml:601-604), and Workspace Studio is a bar widget
// and an overlay, not a menu. Rather than misdescribe the plugin to unlock a
// ranking function, the picker reads the `DesktopEntries` singleton directly
// and ranks with this copy — so the app list is ordered exactly the way the
// Omarchy launcher orders it.
//
// If Omarchy's original changes, re-copy it and re-run tests/appsearch.test.mjs.
